...
opt = zeros(length(C_choice),length(sigma_choice))
for i = 1:length(C_choice),
	for j = 1: length(sigma_choice),
		...
		opt(i,j) = mean(double(pred ~= yval))
	end;
end;
[colMin, colIndex] = min(min(opt)); 
[minValue, rowIndex] = min(opt(:,colIndex));
C = C_choice(rowIndex)
sigma = sigma_choice(colIndex)
% =========================================================================
